...
procedure SYT_INSERT(SNAME:string6; var P:ref);
...
function SYT_SEARCH(SNAME:string6; P:ref): REF;
var FOUND: boolean;
...
function Entry_Search(Name:string6):integer;
var l,r,m : integer;
    nam: string6;

begin
  {for l:= 1 to 6 do nam[l]:= name[l];}
  nam:=name;
  l:=1;
  r:= N_Entries;
...
begin
     if SNAME[1] = '=' then begin  { external Symbol found }
        ...
        if P= nil then begin       { not found in Table }
           ...
           if I = 0 then begin     { not found }
              GET_SYT:= 0;           { function returns zero }
              KNOWN:= NOTFOUND         { flag symbol not found }
           end
           else begin       { something found in external Entry Table }
              Value:= Ext_Entry[i].E_Value;
              PUT_SYT(SNAME,Value,EXTERN); { put sym into Symbol Table }
              GET_SYT:= Value;  { return value }
              KNOWN:= EXTERN;
     ...
     else begin { not external Symbol }
        P:=SYT_SEARCH(SNAME,ROOT);
        if P= nil then begin
           GET_SYT:= 0;
           KNOWN:= NOTFOUND
...
begin
          if P <> nil then
             with P^ do begin
                  PRINTTREE(LEFT);
                  LLINE:=NAME;
                  while length (LLINE) < 6 do LLINE:= LLINE+' ';
                  LLINE:= LLINE+'  ';
                  case TYP of
...
begin
   if(length(LABEL_FIELD)=0) or (not LABEL_OK(LABEL_FIELD)) then P2ERR(22)
   else if (LABEL_FIELD[1] = '=' )then P2ERR(93)
   else begin
      ...
      if PASS1 then begin
         ...
         if P^.TYP <> EQUATE then P2ERR(41)
            ...
            P^.TYP:= EQUPAS2;
            if abs(EX-P^.VALUE) > 0.1 then P2ERR(57)
         end
      end
   end
end;
...
begin
   ...
   WRAPUPOLD;
   if abs(EXPRESSION()-1.0)<0.1 then
...
begin
   if (length(SLINE)=0) or (FIRSTCHAR = '*') then begin
      ...
      else GET_STR(TLINE,7,86,LABEL_FIELD,false,true);
